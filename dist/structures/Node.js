...
class Node {
        ...
        if (!skipFlag &&
            (player.queue.previous.length === 0 ||
                (player.queue.previous[0] &&
                    player.queue.previous[0].track !== player.queue.current?.track))) {
    ...
    async setSponsorBlock(player, segments = [SponsorBlockSegment.Sponsor, SponsorBlockSegment.SelfPromo]) {
        ...
        if (segments.some((v) => !validSponsorBlocks.includes(v.toLowerCase())))
            throw new SyntaxError(`You provided a sponsorblock which isn't valid, valid ones are: ${validSponsorBlocks
                .map((v) => `'${v}'`)
                .join(', ')}`);
